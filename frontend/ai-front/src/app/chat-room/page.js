...
export default function ChatPage() {
  ...
  const switchStatus = useCallback(async () => {
    ...
    try {
      const res = await query("/api/chat/status-switch", {
        method: "post",
        data: data,
      });
    } catch (e) {
  ...
  const getFriends = async () => {
    try {
      const res = await query(`/api/chat/friends/${user?.id}`, {
        method: "get",
      });
      if (res) {
        ...
        const requests = res?.data
          ?.filter((d) => d.inviter_id != user?.id && d.status == "pending")
          .map((d) => {
  ...
  const getMessages = async () => {
    try {
      const res = await query(`/api/chat/messages/${user?.id}`, {
        method: "get",
      });
      if (res?.data) {
  ...
  const selFriend = useMemo(() => {
    let selectedFriend = friendsData?.filter(
      (f) => f.friend_id == friendActiveId
    );
  ...
  const selFriendMessages = useMemo(() => {
    ...
    if (selFriend !== null && userMessages) {
      let friendMessages = userMessages?.filter(
        (data) => data?.conversation_id == selFriend?.id
      );
  ...
  const getMessageData = useCallback(
    (data) => {
      ...
      let selIndex = Array.from(userMessages).findIndex(
        (m) => m.conversation_id == selFriend?.id
      );
  ...
  useEffect(() => {
    // an updater effect to update the local storage when it detectects changes in
    ...
    for (let i = 0; i < last_friend_list?.length; i++) {
      ...
      ref_obj = Array?.from(friendsData).find(
        (d) => d?.friend_id == obj?.friend_id
      );
      for (let key in obj) {
  ...
  const getOpenRequest = useCallback(
    (bool) => {
      if (bool == true) {
        setOpenMobile(true);
      }
  ...
  return (
    <>
      ...
      >
        ...
        >
          <SettingsBar
            ProfileImage={imageRender(image)}
          ...
          >
            ...
            >
              ...
              <CustomSwitch
                ...
                setValue={() => setActivityStatus((prev) => !prev)}
              />
              <Box display="flex" columnGap={"1rem"}>
                <Text>Status:</Text>
                <Text color={activityStatus ? "green.800" : "red.800"}>
            ...
            >
              <Box
                flexBasis={{ base: "10%", md: "30%", lg: "20%" }}
                paddingTop={"1rem"}
                height={{ base: "auto", lg: "100%" }}
                backgroundColor={"blackAlpha.600"}
              >
                <LastChatsCol
                  last_profiles={limitedProfiles}
                  setFriendId={getFriendId}
                  mobile={mobile}
                />
              </Box>
              <Box
                flexBasis={{ base: "90%", lg: "80%" }}
              ...
              >
                <Motion open={openChat} h={"inherit"} w="inherit">
                  <ChatMainCol
                    ...
                    user={user}
                    friend={selFriend}
                    messagesData={selFriendMessages}
                    profile={profile}
