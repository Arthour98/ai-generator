...
export default function RadioPageClient({ initialCountries, initialTags }) {
  ...
  const getAndHandleIndex = useCallback(
    (i) => {
  ...
  const submitQuery = useCallback(async () => {
    ...
    if (
      ...
      showToast({
        status: "error",
        content: "You have to disselect one of searching options",
      });
    }
    if (selCountry !== "" && (selTag === "" || selTag == "None")) {
      ...
      const req = await query(`/api/radio/country/sel`, {
        method: "post",
        data: data,
      });
      if (req) {
      ...
      const req = await query("/api/radio/tag/sel", {
        method: "post",
        data: data,
      });
      if (req) {
