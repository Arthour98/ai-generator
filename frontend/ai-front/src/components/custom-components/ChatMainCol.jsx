...
export default function ChatMainCol({
  friend,
  open,
  setOpen,
  user,
  ...
  const sendMessage = useCallback(
    async (e) => {
      if (e.key == "Enter" && message != "") {
        ...
        try {
          const res = await query("/api/chat/send-message", {
            method: "post",
            data: data,
          });
          if (res.success == true) {
  ...
  return (
    ...
    >
      <Box display="flex" justifyContent="center" alignItems="center" h="10%">
        <Text fontSize="26px" fontWeight="500" color="whiteAlpha.300">
          {capitalize(friend_obj?.nickname)}
        </Text>
      </Box>
      <Box
        display="flex"
