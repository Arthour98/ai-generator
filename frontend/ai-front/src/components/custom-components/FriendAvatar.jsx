...
export default function FriendAvatar({
  ...
  const sendInvite = async (id) => {
    ...
    try {
      const res = await query("/api/chat/send-friend-request", {
        data: data,
        method: "post",
      });
      if (res.message === "sent") {
        CustomToast({
          content: "Your invitation was sent successfully",
          status: "success",
        });
        popItem(id);
        ...
        CustomToast({
          content: "You need to set up your profile first!",
          status: "error",
        });
        popItem(id);
      }
  ...
  const acceptInvite = async () => {
    ...
    try {
      const res = await query("/api/chat/accept-friend-request", {
        method: "post",
        data: data,
      });
      if (res.message) {
        CustomToast({
          content: "You Accepted Friend Request",
          status: "success",
        });
        popItem(id);
      }
  ...
  const declineInvite = async () => {
    ...
    try {
      ...
      if (res.message) {
        CustomToast({
          content: "You Declined Friend Request",
          status: "success",
        });
        popItem(id);
      }
  ...
  const deleteFriend = async () => {
    ...
    try {
      const res = await query("/api/chat/delete-friend", {
        method: "post",
        data: data,
      });
      if (res.message == "success") {
        CustomToast({
          status: "success",
          content: "You successfuly deleted this person",
        });
        popItem(id);
      }
    } catch (e) {
      CustomToast({
        status: "error",
        content: "There was an error deleting this person",
      });
    }
  };
  ...
  return (
    <Box
      ...
      boxShadow={
        isHovering
          ? "0 0 15px var(--chakra-colors-green-200)"
          : "0 0 10px var(--chakra-colors-green-400)"
      }
      position="relative"
