...
export default function LastChatsCol({ setFriendId, last_profiles, mobile }) {
  ...
  }, [last_profiles]); // the state that gonna hold the profiles fetched from local storage
  useEffect(() => {
    ...
    >
      {limitedProfiles.map((prof) => (
        <Box
          ...
          display="flex"
          borderRadius={"7px"}
          onClick={() => getFriendId(prof?.friend_id)}
        ...
        >
          ...
          >
            <CustomAvatar
              src={imageRender(prof?.image_profile)}
              noScale
              w="1rem"
              h="1rem"
            />
