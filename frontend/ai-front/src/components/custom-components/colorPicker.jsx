...
export default function ColorPicker({ w, h, radius, initialColor, top, left, label, value, setValue }) {
    ...
    <Box display={"flex"} gap={"10px"}>
      ...
      <Box w={w} h={h} bg={"white"} onClick={() => setOpen(true)}
        ...
        {open &&
          ...
          >
            <HexColorPicker color={value ?? color}
              onChange={(color) => { setColor(color); setValue(color) }}
              style=
              {{
                position: "absolute",
                top: top ? `${top}%` : "100%",
