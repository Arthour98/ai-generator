...
function FriendsBar({
  ...
  const getItemToPop = useCallback(
    (id) => {
  ...
  const searchFriendtoAdd = useCallback(async () => {
    ...
    try {
      const res = await query("/api/profile/searchProfiles", {
        data: data,
        method: "post",
      });
      if (res) {
  ...
  return (
    <Box
      ...
      height="100vh"
      position="fixed"
      background={isMobile ? "black" : "blackAlpha.500"}
      top="0"
      right="0"
    >
      {isMobile && (
        ...
        <Box className={styles.iconAndActionWrapper}>
          <Box className={styles.iconBar}>
            ...
            <FontAwesomeIcon
              ...
              style={{
                padding: "0.3rem",
                backgroundColor: friendsMode?.includes("display_friends")
                  ? "rgba(0,0,0,0.2"
                  : "inherit",
                borderRadius: "6px",
              }}
            ...
            {friendsMode?.includes("display_friends") ? (
              <Box className={styles.displayFriendsWrapper}>
                ...
                <Box
                  ...
                  _hover={{ backgroundColor: "gray.700" }}
                  onClick={() =>
                    setFriendsMode("display_friends-requests_view")
                  }
                  style={{
                    backgroundColor:
                      friendsMode == "display_friends-requests_view"
                        ? "rgba(0,0,0,0.2"
                        : "transparent",
                  }}
                  position="relative"
